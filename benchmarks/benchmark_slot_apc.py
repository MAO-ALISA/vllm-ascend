...
def run(length: int, repeats: int) -> None:
    ...
    for ratio in (4, 128):
        ...
        pool = BlockPool(length // (128 * ratio) + 3, True, 8)
        ...
        for boundary in sorted({128, 384, length - 128}):
            ...
            print(f"C{ratio} cached={length} hit={boundary}: lookup={lookup_us:.2f} us; publish={publish_ms:.2f} ms")
